//! PrizePicks market price / spread snapshot history.

use serde::{Deserialize, Serialize};
use sqlx::{Pool, Row, Sqlite};

use super::models::PrizePicksMarketSummary;

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksPriceSnapshot {
    pub id: String,
    pub ticker: String,
    pub title: String,
    pub category: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksPriceHistory {
    pub ticker: String,
    pub snapshots: Vec<PrizePicksPriceSnapshot>,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksSnapshotBatch {
...
pub async fn init_price_tables(pool: &Pool<Sqlite>) -> Result<(), String> {
    ...
    .execute(pool)
    .await
    .map_err(|e| format!("prizepicks_price_snapshots create failed: {}", e))?;

    sqlx::query("CREATE INDEX IF NOT EXISTS idx_kps_ticker ON prizepicks_price_snapshots(ticker)")
        .execute(pool)
        .await
        .ok();
    sqlx::query(
        "CREATE INDEX IF NOT EXISTS idx_kps_snapshot ON prizepicks_price_snapshots(snapshot_at)",
    )
    .execute(pool)
    .await
    .ok();
...
pub async fn snapshot_markets(
    pool: &Pool<Sqlite>,
    markets: &[PrizePicksMarketSummary],
) -> Result<PrizePicksSnapshotBatch, String> {
    ...
    for m in markets {
        ...
        let rows = sqlx::query(
            r#"
            INSERT OR IGNORE INTO prizepicks_price_snapshots
        ...
        .execute(pool)
        .await
        .map_err(|e| format!("insert prizepicks snapshot: {}", e))?
        .rows_affected();
...
pub async fn get_price_history(
    ...
    ticker: &str,
    limit: i64,
) -> Result<PrizePicksPriceHistory, String> {
    let rows = sqlx::query(
        r#"
    ...
    .fetch_all(pool)
    .await
    .map_err(|e| format!("fetch prizepicks price history: {}", e))?;

    let snapshots: Vec<PrizePicksPriceSnapshot> = rows
        .iter()
        .map(|r| PrizePicksPriceSnapshot {
            id: r.get("id"),
            ticker: r.get("ticker"),
            title: r.get("title"),
...
pub async fn prune_old_snapshots(pool: &Pool<Sqlite>, keep_days: i64) -> Result<u64, String> {
    ...
    let rows = sqlx::query("DELETE FROM prizepicks_price_snapshots WHERE snapshot_at < ?1")
        .bind(cutoff.to_rfc3339())
        .execute(pool)
        .await
        .map_err(|e| format!("prune prizepicks snapshots: {}", e))?
        .rows_affected();
    Ok(rows)
}
