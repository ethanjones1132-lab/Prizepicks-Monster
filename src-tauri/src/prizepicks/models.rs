...
// ═══════════════════════════════════════════════════════════════
// PrizePicks Trading API v2 — Data Models
// Base URL: https://trading-api.prizepicks.com/trade-api/v2
// ═══════════════════════════════════════════════════════════════

/// PrizePicks client configuration
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksConfig {
    ...
    /// Use the PrizePicks demo environment
    pub use_demo: bool,
}

impl Default for PrizePicksConfig {
    fn default() -> Self {
        PrizePicksConfig {
            base_url: "https://trading-api.prizepicks.com/trade-api/v2".to_string(),
            email: String::new(),
            password: String::new(),
...
/// A single PrizePicks binary market
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct PrizePicksMarket {
...
impl PrizePicksMarket {
    ...
    /// Infer a display category from PrizePicks metadata, then fall back to ticker prefixes.
    pub fn infer_category(&self) -> &'static str {
        ...
        if t.contains("NBA")
            ...
            || t.contains("PGA")
            || t.contains("MATCH")
            || t.contains("SETWINNER")
            || t.contains("EXACTMATCH")
        {
            "Sports"
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksMarketsResponse {
    pub cursor: Option<String>,
    #[serde(default)]
    pub markets: Vec<PrizePicksMarket>,
}

/// A single PrizePicks event (a group of related markets)
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct PrizePicksEvent {
    pub event_ticker: String,
    #[serde(default)]
    pub title: String,
    #[serde(default)]
    ...
    pub markets: Option<Vec<PrizePicksMarket>>,
    #[serde(default)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksEventsResponse {
    pub cursor: Option<String>,
    #[serde(default)]
    pub events: Vec<PrizePicksEvent>,
}

/// Single orderbook level
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksOrderbookLevel {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksOrderbook {
    pub yes: Vec<PrizePicksOrderbookLevel>,
    pub no: Vec<PrizePicksOrderbookLevel>,
}

/// Wrapped orderbook response
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksOrderbookResponse {
    pub orderbook: PrizePicksOrderbook,
}

/// Portfolio balance (requires auth)
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksBalance {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksBalanceResponse {
    pub balance: PrizePicksBalance,
}

/// An open position
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksPosition {
    pub ticker: String,
...
/// Positions response
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksPositionsResponse {
    pub cursor: Option<String>,
    #[serde(default)]
    pub market_positions: Vec<PrizePicksPosition>,
}

/// Condensed market data returned to the frontend
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksMarketSummary {
    pub ticker: String,
    pub event_ticker: String,
    pub title: String,
    pub category: String,
...
impl From<&PrizePicksMarket> for PrizePicksMarketSummary {
    fn from(m: &PrizePicksMarket) -> Self {
...
pub struct PrizePicksMarketsQuery {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksPrediction {
    pub id: String,
    pub ticker: String,
    pub title: String,
    pub category: String,
    ...
    /// Player prop line (e.g., 275.5 for "Over 275.5 Passing Yards") — used for Over/Under grading
    pub line: Option<f64>,
    /// Actual stat value at game end (e.g., 289.0 for "Mahomes threw 289 yards") — used for Over/Under grading
    pub actual_stat_value: Option<f64>,
    /// Payout multiplier for this pick (e.g., 3.0 for 2-pick Power Play)
    pub multiplier: Option<f64>,
}

/// Stats for PrizePicks predictions
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksPredictionStats {
    pub total: u32,
    pub wins: u32,
    pub losses: u32,
    pub pending: u32,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksGradingResult {
...
// ── Shared betting-side types ──

/// Side of a bet (binary or Over/Under).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum PrizePicksBetSide {
    Yes,
    No,
    Over,
    Under,
...
/// Parse bet side from contract_side + pick_type fields.
pub fn parse_bet_side(contract_side: Option<&str>, pick_type: Option<&str>) -> PrizePicksBetSide {
    if let Some(raw) = contract_side {
        match raw.trim().to_uppercase().as_str() {
            "YES" => return PrizePicksBetSide::Yes,
            "NO" => return PrizePicksBetSide::No,
            "OVER" | "O" | "MORE" => return PrizePicksBetSide::Over,
            "UNDER" | "U" | "LESS" => return PrizePicksBetSide::Under,
            "PASS" => return PrizePicksBetSide::Pass,
            _ => {}
        }
    }
    if let Some(pt) = pick_type {
        match pt.trim().to_lowercase().as_str() {
            "over" | "o" | "more" => return PrizePicksBetSide::Over,
            "under" | "u" | "less" => return PrizePicksBetSide::Under,
            _ => {}
        }
    }
    PrizePicksBetSide::Unknown
}

/// Summary of a grading run
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PrizePicksGradingSummary {
