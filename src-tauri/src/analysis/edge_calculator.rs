//! Edge Calculator — inline copy of the former `monster-edge-core` crate.
//!
//! The shared `monster-edge-core` crate was inlined into kalshi-monster and
//! its source directory deleted. This module provides the same types and
//! functions (EdgeScore, calculate_edge, apply_calibration, embedded_calibrator)
//! that the rest of the app imports through `crate::analysis::edge_calculator::*`.
...
/// Load the isotonic calibrator artifact embedded at build time.
///
/// The calibrator was fitted offline and checked in as `calibrator.json`
/// alongside this module. Returns `None` if the artifact is missing or
/// malformed (treated as "no calibration available").
