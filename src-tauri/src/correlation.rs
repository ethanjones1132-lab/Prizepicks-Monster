...
pub fn analyze_correlation(picks: &[CorrelationPick]) -> CorrelationAnalysis {
    ...
    let overall_correlation_score = if !pair_correlations.is_empty() {
        let sum: f64 = pair_correlations
            .iter()
            .map(|pc| pc.correlation_score.abs())
            .sum();
    ...
    if overall_correlation_score > 0.5 {
        suggestions.push(
            "Consider removing highly correlated legs — they don't provide true diversification."
                .to_string(),
        );
        suggestions.push(
            "Look for props from different games or different game scripts to reduce correlation."
                .to_string(),
        );
    }
...
fn analyze_game_script(
    ...
    for (_game_key, indices) in game_groups {
        ...
        let players: Vec<String> = indices
            .iter()
            .map(|&i| picks[i].player_name.clone())
            .collect();
...
pub fn optimize_parlay(picks: &[CorrelationPick], max_correlation: f64) -> ParlayOptimization {
...
mod tests {
    ...
    fn test_same_team_qb_wr_correlation() {
        ...
        assert_eq!(
            analysis.pair_correlations[0].correlation_type,
            "same_team_offensive"
        );
    }
