...
/// Base directory for all PrizePicks Monster data
fn data_dir() -> PathBuf {
                ...
                if let Ok(mut session) = serde_json::from_str::<ChatSession>(&content) {
                    ...
                    if let Ok(msgs) = load_session_messages(&session.id) {
                        ...
                        session.total_tokens =
                            msgs.iter().filter_map(|m| m.tokens_used).sum::<u64>();
                    }
...
pub fn save_session_messages(session_id: &str, messages: &[ChatMessage]) -> Result<(), String> {
    let dir = ensure_sessions_dir()?;
...
pub fn load_session_messages(session_id: &str) -> Result<Vec<ChatMessage>, String> {
    ...
    let content =
        fs::read_to_string(&path).map_err(|e| format!("Failed to read messages: {}", e))?;
