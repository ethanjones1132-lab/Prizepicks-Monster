...
pub async fn send_discord_webhook(url: &str, content: &str) -> Result<(), String> {
    ...
    let payload = DiscordWebhookPayload {
        ...
        username: Some("PrizePicks Monster".to_string()),
        avatar_url: None,
        embeds: vec![],
    };
...
pub async fn send_discord_embed(
    ...
    let embed = DiscordEmbed {
        ...
        color,
        fields: embed_fields,
        footer: footer.map(|f| DiscordEmbedFooter {
            text: f.to_string(),
        }),
...
pub async fn send_bot_notification(
    ...
    let type_enabled = match notification_type {
        ...
        "prediction_graded" | "prediction_win" | "prediction_loss" | "prediction_push"
        | "grading_complete" => config.preferences.grading_results_enabled,
