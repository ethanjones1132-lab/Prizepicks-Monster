...
use super::storage;
use crate::chat::decision_schema::{ContractSide, PrizePicksTradeDecision};
use crate::prizepicks::grading::{infer_market_price_at_entry, resolved_bet_won};
use crate::prizepicks::models::{
    CalibrationMetrics, CategoryStats, PrizePicksGradingResult, PrizePicksGradingSummary,
    PrizePicksPrediction, PrizePicksPredictionStats,
};
...
pub struct Prediction {
    ...
    /// Serialized `PrizePicksTradeDecision` JSON when the prediction is a PrizePicks market trade.
...
impl PredictionTracker {
        ...
        while let Some(block_start) = text[search_start..].find("```json") {
            ...
            if let Some(block_end) = text[abs_start..].find("```") {
                ...
                if let Ok(val) = serde_json::from_str::<serde_json::Value>(trimmed) {
                    if let Some(pred) =
                        self.parse_prizepicks_json_prediction(session_id, &val, now, text)
                    {
                ...
                if let Ok(val) = serde_json::from_str::<Vec<serde_json::Value>>(trimmed) {
                    for item in &val {
                        if let Some(pred) =
                            self.parse_prizepicks_json_prediction(session_id, item, now, text)
                        {
                            predictions.push(pred);
                        } else if let Some(pred) =
    ...
    fn is_prizepicks_trade_json(val: &serde_json::Value) -> bool {
        val.get("ticker")
            .and_then(|v| v.as_str())
    ...
    fn parse_prizepicks_json_prediction(
        &self,
        session_id: &str,
        val: &serde_json::Value,
        now: &str,
        ...
        if !Self::is_prizepicks_trade_json(val) {
            return None;
        }
        let decision: PrizePicksTradeDecision = serde_json::from_value(val.clone()).ok()?;
    ...
    fn parse_pick_details(rest: &str, prediction: &mut Prediction) {
        ...
        if parts.len() >= 2 {
            if let Ok(line_val) = parts[0].trim().parse::<f64>() {
                ...
                let clean_price: String = parts[0]
                    .chars()
    ...
    pub async fn get_all_predictions(&self) -> Vec<PredictionRecord> {
        storage::get_all_predictions(&self.pool)
            .await
            .unwrap_or_default()
    }
    ...
    pub fn get_stats(&self, records: &[PredictionRecord]) -> PredictionStats {
        ...
        let wins = all
            .iter()
            .filter(|r| r.outcome == PredictionOutcome::Win)
            .count() as u32;
        let losses = all
            .iter()
            .filter(|r| r.outcome == PredictionOutcome::Loss)
            .count() as u32;
        let pushes = all
            .iter()
            .filter(|r| r.outcome == PredictionOutcome::Push)
            .count() as u32;
        let pending = all
            .iter()
        ...
        let win_rate = if decided > 0.0 {
            wins as f64 / decided * 100.0
        } else {
            0.0
        };

        let scored: Vec<u8> = all
            .iter()
            .filter_map(|r| r.prediction.confidence_score)
            .collect();
        ...
        let high_conf: Vec<&PredictionRecord> = all
            .iter()
            .filter(|r| {
                r.prediction.confidence.as_deref() == Some("High")
                    && r.outcome != PredictionOutcome::Pending
            })
            .collect();
        let med_conf: Vec<&PredictionRecord> = all
            .iter()
            .filter(|r| {
                r.prediction.confidence.as_deref() == Some("Medium")
                    && r.outcome != PredictionOutcome::Pending
            })
            .collect();
        let low_conf: Vec<&PredictionRecord> = all
            .iter()
            .filter(|r| {
                r.prediction.confidence.as_deref() == Some("Low")
                    && r.outcome != PredictionOutcome::Pending
            })
            .collect();
        ...
        for (i, (lo, hi)) in ranges.iter().enumerate() {
            ...
            let avg_score = if count > 0 {
                bucket
                    .iter()
                    .filter_map(|r| r.prediction.confidence_score.map(|s| s as f64))
                    .sum::<f64>()
                    / count as f64
            } else {
                0.0
        ...
        PredictionStats {
            ...
            high_confidence_wins: high_conf
                .iter()
                .filter(|r| r.outcome == PredictionOutcome::Win)
                .count() as u32,
            high_confidence_total: high_conf.len() as u32,
            medium_confidence_wins: med_conf
                .iter()
                .filter(|r| r.outcome == PredictionOutcome::Win)
                .count() as u32,
            medium_confidence_total: med_conf.len() as u32,
            low_confidence_wins: low_conf
                .iter()
                .filter(|r| r.outcome == PredictionOutcome::Win)
                .count() as u32,
    ...
    fn compute_trend_data(records: &[&PredictionRecord]) -> Vec<TrendDataPoint> {
        ...
        let mut buckets: std::collections::HashMap<String, Vec<&PredictionRecord>> =
    ...
    pub async fn get_player_trend(&self, player_name: &str) -> Option<PlayerTrend> {
        ...
        let wins = player_records
            .iter()
            .filter(|r| r.outcome == PredictionOutcome::Win)
            .count() as u32;
        let losses = player_records
            .iter()
            .filter(|r| r.outcome == PredictionOutcome::Loss)
            .count() as u32;
        let decided = (wins + losses) as f64;
        let overall_win_rate = if decided > 0.0 {
            wins as f64 / decided * 100.0
        } else {
            0.0
        };
    ...
    pub async fn get_stat_category_trend(&self, stat_category: &str) -> Option<StatCategoryTrend> {
        ...
        let wins = cat_records
            .iter()
            .filter(|r| r.outcome == PredictionOutcome::Win)
            .count() as u32;
        let losses = cat_records
            .iter()
            .filter(|r| r.outcome == PredictionOutcome::Loss)
            .count() as u32;
        let decided = (wins + losses) as f64;
    ...
    pub async fn get_overall_trend(&self) -> OverallTrend {
        ...
        let mut player_map: std::collections::HashMap<String, (u32, u32)> =
            std::collections::HashMap::new();
        for r in &resolved {
    ...
    fn parse_prizepicks_decision_blob(json: &str) -> Option<PrizePicksTradeDecision> {
        serde_json::from_str::<PrizePicksTradeDecision>(json).ok()
    }

    fn prizepicks_decision_from_record(r: &PredictionRecord) -> Option<PrizePicksTradeDecision> {
        if let Some(ref blob) = r.prediction.full_decision_json {
            if let Some(d) = Self::parse_prizepicks_decision_blob(blob) {
                return Some(d);
            }
        }
        if let Some(ref notes) = r.notes {
            if notes.trim_start().starts_with('{') {
                if let Some(d) = Self::parse_prizepicks_decision_blob(notes) {
                    return Some(d);
                }
            }
        }
        Self::find_prizepicks_decision_in_text(&r.prediction.raw_text)
    }

    fn find_prizepicks_decision_in_text(text: &str) -> Option<PrizePicksTradeDecision> {
        let mut search_start = 0;
        while let Some(block_start) = text[search_start..].find("```json") {
            let abs_start = search_start + block_start + 7;
            if let Some(block_end) = text[abs_start..].find("```") {
    ...
    fn record_is_prizepicks(r: &PredictionRecord) -> bool {
        r.prediction.full_decision_json.is_some()
            || Self::prizepicks_decision_from_record(r).is_some()
            || r.prediction
                .player_name
                .as_deref()
                .map_or(false, |n| n.starts_with("KX") || n.contains('-'))
            || r.prediction.raw_text.to_lowercase().contains("prizepicks")
    }

    fn prizepicks_prediction_from_record(r: &PredictionRecord) -> Option<PrizePicksPrediction> {
        if !Self::record_is_prizepicks(r) {
            return None;
        }

        let decision = Self::prizepicks_decision_from_record(r);
        let ticker = decision
            .as_ref()
        ...
        let (
            ...
            (
                ...
                Some(d.risk_flags.iter().map(|f| format!("{:?}", f)).collect()),
    ...
    pub async fn get_prizepicks_predictions(&self) -> Vec<PrizePicksPrediction> {
        let all = self.get_all_predictions().await;
        all.iter()
            .filter_map(Self::prizepicks_prediction_from_record)
            .collect()
    }

    pub async fn update_prizepicks_outcome(
        &self,
        prediction_id: &str,
        actual_outcome: &str,
        pnl: f64,
        ...
        .execute(&self.pool)
        .await
        .map_err(|e| format!("Failed to update PrizePicks outcome: {}", e))?
        .rows_affected();
    ...
    pub async fn get_prizepicks_stats(
        &self,
        predictions: &[PrizePicksPrediction],
    ) -> PrizePicksPredictionStats {
        ...
        let scored: Vec<u8> = predictions
            .iter()
            .filter_map(|p| p.confidence_score)
            .collect();
        let avg_confidence_score = if !scored.is_empty() {
            scored.iter().map(|&s| s as f64).sum::<f64>() / scored.len() as f64
        } else {
        ...
        let roi_pct = if total_volume_traded > 0.0 {
            total_pnl / total_volume_traded * 100.0
        } else {
            0.0
        };

        let resolved: Vec<&PrizePicksPrediction> = predictions
            .iter()
        ...
        let brier_score = if resolved.is_empty() {
            ...
            let sum_sq_error: f64 = resolved
                .iter()
                .map(|p| {
                    let actual = if p.actual_outcome.as_deref() == Some("Yes") {
                        1.0
                    } else {
                        0.0
                    };
        ...
        for p in predictions {
            ...
            if p.actual_outcome.is_some() {
                ...
                entry.win_rate = if dec > 0.0 {
                    entry.wins as f64 / dec * 100.0
                } else {
                    0.0
                };
            }
        ...
        PrizePicksPredictionStats {
            ...
            total_pnl,
            roi_pct,
            calibration: crate::prizepicks::models::CalibrationMetrics {
                brier_score,
    ...
    pub async fn get_prizepicks_grading_summary(&self) -> PrizePicksGradingSummary {
        let preds = self.get_prizepicks_predictions().await;
        let resolved: Vec<PrizePicksPrediction> = preds
            .into_iter()
            .filter(|p| p.actual_outcome.is_some())
            .collect();
...
mod tests {
    ...
    fn test_extract_json_prediction() {
        ...
        let tracker = prediction_tracker_extraction_only();
        let text = r#"```json
{
...
```"#;
    ...
    fn prediction_tracker_extraction_only() -> PredictionTracker {
