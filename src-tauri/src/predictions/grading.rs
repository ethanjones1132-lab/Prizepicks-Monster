...
const STAT_CATEGORY_MAP: &[(&str, &[&str])] = &[
    ...
    (
        "Passing Yards",
        &["passingYards", "passYards", "passing yards"],
    ),
    (
        "Rushing Yards",
        &["rushingYards", "rushYards", "rushing yards"],
    ),
    (
        "Receiving Yards",
    ...
    (
        "Passing Touchdowns",
        &["passingTouchdowns", "passingTd", "passing touchdowns"],
    ),
    (
        "Rushing Touchdowns",
        &["rushingTouchdowns", "rushingTd", "rushing touchdowns"],
    ),
    (
        "Receiving Touchdowns",
        &["receivingTouchdowns", "receivingTd", "receiving touchdowns"],
    ),
    (
        "Receptions",
    ...
    (
        "Passing Attempts",
        &["passingAttempts", "passAttempts", "passing attempts"],
    ),
    (
        "Rushing Attempts",
        &[
    ...
    (
        "Longest Reception",
        &["longestReception", "longRec", "longest reception"],
    ),
    ("Targets", &["targets", "receivingTargets", "targets"]),
    (
        "Total Touchdowns",
        &["totalTouchdowns", "totalTd", "total touchdowns"],
    ),
    (
        "Total Yards",
        &["totalYards", "total yards", "yardsFromScrimmage"],
    ),
];
...
pub async fn fetch_game_boxscore(
        ...
        .and_then(|p| p.as_array())
    {
        for team_entry in players_section {
            if let Some(statistics) = team_entry.get("statistics").and_then(|s| s.as_array()) {
                for stat_category in statistics {
                    ...
                    if let Some(athletes) = stat_category.get("athletes").and_then(|a| a.as_array())
                    {
                        for athlete in athletes {
                            ...
                            let stats = athlete.get("stats").and_then(|s| s.as_array());
    ...
    if player_stats.is_empty() {
            ...
            .and_then(|r| r.as_array())
        {
            for team_roster in rosters {
                if let Some(players) = team_roster.get("players").and_then(|p| p.as_array()) {
                    for player in players {
                        ...
                        if let Some(stats_obj) =
                            player.get("statistics").and_then(|s| s.as_object())
                        {
                            let entry = player_stats
                                .entry(name.to_string())
                                .or_insert_with(HashMap::new);
...
fn name_matches_fuzzy(name1: &str, name2: &str) -> bool {
    ...
    let parts1: Vec<&str> = name1
        .split(|c| c == ',' || c == ' ')
        .filter(|s| !s.is_empty())
        .collect();
    let parts2: Vec<&str> = name2
        .split(|c| c == ',' || c == ' ')
        .filter(|s| !s.is_empty())
        .collect();
    ...
    for (pred_id, player, pick_type, line, stat_cat, _) in pending {
        ...
        for (game_id, boxscore) in &all_boxscores {
            let result = grade_prediction(
                pred_id, player, pick_type, *line, stat_cat, boxscore, game_id, "Final",
            );
...
mod tests {
    ...
    fn test_normalize_category() {
        ...
        assert_eq!(
            normalize_category("Passing Touchdowns"),
            "Passing Touchdowns"
        );
    }
