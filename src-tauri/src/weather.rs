...
impl WeatherClient {
    ...
    pub async fn get_weather(&mut self, game: &str, location: &str) -> Result<GameWeather, String> {
        // Check cache
            ...
            Err(e) => {
                ...
                if !self.openweathermap_api_key.is_empty() {
                    match self.fetch_openweathermap(location).await {
                        ...
                        Err(e2) => {
                            ...
                            GameWeather {
                                ...
                                is_dome: false,
                                impact_summary:
                                    "Weather data unavailable — assume neutral conditions".into(),
                            }
                        }
                    }
                } else {
                    GameWeather {
                        ...
                        condition: "Unknown".into(),
                        is_dome: false,
                        impact_summary: "Weather data unavailable — assume neutral conditions"
                            .into(),
                    }
                }
            }
        ...
        self.cache
            .insert(location.to_string(), (weather.clone(), now));
        Ok(weather)
    }
...
fn assess_weather_impact(
    ...
    if let Some(w) = wind {
        if w >= 25.0 {
            ...
            impacts.push(
                "moderate wind — some suppression of passing/kicking, slight lean toward unders",
            );
        } else if w >= 10.0 {
