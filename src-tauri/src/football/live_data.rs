...
use crate::football::data::{
    get_football_context, get_multi_sport_context, FootballContext, GameInfo, MultiSportContext,
    PlayerProfile,
};
use chrono::Utc;
use reqwest::Client;
...
pub async fn fetch_live_scoreboard_for_league(
    league: SportLeague,
    ...
    let payload: Value = response.json().await.map_err(|e| {
        format!(
            "Failed to parse {} scoreboard JSON: {}",
            league.short_name(),
            e
        )
    })?;
            ...
            if !relevant_players.is_empty() {
                ...
                for player in &relevant_players {
                    let _ = writeln!(
                        out,
                        "  {} ({}) {} — {}",
                        player.name, player.team, player.position, player.note
                    );
                    let _ = writeln!(
                        out,
                        "    season: {}",
                        format_stat_pairs(&player.season_avg_game)
                    );
...
fn select_relevant_multi_sport_players(
    ctx: &MultiSportContext,
    query_text: &str,
    max_players: usize,
) -> Vec<LivePlayerSnapshot> {
    let tokens = tokenize(query_text);
    let mut ranked: Vec<(i32, &PlayerProfile)> = ctx
        .top_players
        .iter()
    ...
    if selected.is_empty() {
        selected.extend(ctx.top_players.iter().take(max_players).map(to_snapshot));
    }
    selected
}
...
pub async fn build_live_data_context(query_text: &str, max_players: usize) -> String {
    ...
    if relevant_players.is_empty() {
        let _ = writeln!(
            out,
            "  No query match found; using the built-in top-player reference set."
        );
    }
    for player in &relevant_players {
        let _ = writeln!(
    ...
    for defense in context.defense_rankings.iter().take(8) {
        let _ = writeln!(
            ...
            "  {} pass#{}/rush#{}/pts#{}: {}",
            defense.team,
            defense.pass_def_rank,
            defense.rush_def_rank,
...
fn summarize_query(query_text: &str) -> String {
    let tokens = tokenize(query_text);
    if tokens.is_empty() {
        return "general NFL prop analysis".into();
    }
...
fn select_relevant_players(
    context: &FootballContext,
    query_text: &str,
    max_players: usize,
) -> Vec<LivePlayerSnapshot> {
    let tokens = tokenize(query_text);
    let mut ranked: Vec<(i32, &PlayerProfile)> = context
        .top_qbs
        .iter()
