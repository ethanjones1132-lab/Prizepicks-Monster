...
fn parse_season_leaders(
    payload: &Value,
    league: SportLeague,
    season: u32,
) -> Vec<PlayerStatProfile> {
        ...
        for leader in leaders.iter().take(10) {
            ...
            if let Some(existing) = profiles.iter_mut().find(|p| p.name == name) {
                if !stat_key.is_empty() {
                    existing
                        .per_game_stats
                        .insert(stat_key.to_string(), stat_val);
                }
...
fn parse_season_leaders_map(payload: &Value, league: SportLeague, season: u32) -> SeasonLeaders {
...
fn parse_team_roster(
    ...
    if let Some(athletes) = payload.get("athletes").and_then(|a| a.as_array()) {
        for athlete in athletes {
            ...
            let id = athlete.get("id").and_then(|v| v.as_str()).unwrap_or("");
...
pub fn build_live_player_context(
    ...
    let _ = writeln!(
        out,
        "\n📊 LIVE PLAYER STATS ({}, {} season):",
        league.short_name(),
        profiles.first().map(|p| p.season).unwrap_or(2025)
    );
    let _ = writeln!(
        out,
        "Source: ESPN | Fetched: {}",
