...
impl SportsApiClient {
    ...
    pub async fn espn_scoreboard(&self) -> Result<Value, String> {
        ...
        if let Some(cached) = self.cache.get(cache_key).await {
            return serde_json::from_str(&cached).map_err(|e| format!("Cache parse error: {}", e));
        }
    ...
    pub async fn api_sports_standings(&self) -> Result<Value, String> {
        ...
        if !resp.status().is_success() {
            return Err(format!(
                "API-Sports standings returned HTTP {}",
                resp.status()
            ));
        }
    ...
    pub async fn espn_sport_player_stats(
        ...
        let cache_key = format!(
            "espn_{}_player_stats_{}_{}",
            sport_path.replace('/', "_"),
            athlete_id,
            season
        );
        if let Some(cached) = self.cache.get(&cache_key).await {
        ...
        if !resp.status().is_success() {
            return Err(format!(
                "ESPN {} player stats returned HTTP {}",
                sport_path,
                resp.status()
            ));
        }
    ...
    pub async fn espn_sport_team_roster(
        ...
        if !resp.status().is_success() {
            return Err(format!(
                "ESPN {} team roster returned HTTP {}",
                sport_path,
                resp.status()
            ));
        }
    ...
    pub async fn espn_sport_season_leaders(
        ...
        if !resp.status().is_success() {
            return Err(format!(
                "ESPN {} season leaders returned HTTP {}",
                sport_path,
                resp.status()
            ));
        }
        ...
        Err(_) => {
            return vec![
                ...
                DataSourceStatus {
                    ...
                    description: "Optional paid API for structured NFL data (100 req/day free)"
                        .into(),
                },
            ];
        }
...
fn filter_injured_players(players: &Value) -> Vec<Value> {
    ...
    if let Some(obj) = players.as_object() {
        for (_id, player) in obj {
            let injury_status = player.get("injury_status").and_then(|v| v.as_str());
            ...
            if has_injury {
                let name = player
                    ...
                    .and_then(|v| v.as_str())
                    .unwrap_or("Unknown");
                let team = player.get("team").and_then(|v| v.as_str()).unwrap_or("UNK");
                let position = player
                    .get("position")
                    .and_then(|v| v.as_str())
                    .unwrap_or("UNK");
