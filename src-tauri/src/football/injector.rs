...
pub async fn build_live_data_packet(
    ...
    let (schedule_result, standings_result, news_result, sleeper_state_result, injuries_result) = tokio::join!(
    ...
    let (injury_context, injury_errors) = match injuries_result {
        Ok(injuries) => (
            format_injury_context(&injuries, &query_teams, &query_players),
            vec![],
        ),
    ...
    let (news_context, news_errors) = match news_result {
        Ok(news) => (
            format_news_context(&news, &query_teams, &query_players),
            vec![],
        ),
    ...
    if !injury_context.is_empty() {
        data_sources.push("Sleeper".into());
    }
    ...
    if !other.is_empty() && relevant.len() < 6 {
        ...
        for game in other_slice.iter().copied().take(6) {
            writeln!(
                ctx,
                "  - {} @ {} ({}) {}",
                game.away_abbr, game.home_abbr, game.game_time, game.status
            )
            .unwrap();
        }
    }
...
fn format_standings_context(standings: &Value) -> String {
    ...
    for entry in entries.iter().take(4) {
        ...
        let entries_inner = entry
            .get("standings")
            .or_else(|| entry.get("entries"))
...
fn format_injury_context(
    ...
    let (relevant, other): (Vec<&Value>, Vec<&Value>) =
        if query_teams.is_empty() && query_players.is_empty() {
            let (first_20, rest) = all_injuries.split_at(all_injuries.len().min(20));
            (first_20.to_vec(), rest.to_vec())
        } else {
            let relevant: Vec<&Value> = all_injuries
                .iter()
                .filter(|inj| {
                    ...
                    query_teams
                        .iter()
                        .any(|t| team.to_uppercase() == t.to_uppercase())
                        || query_players
                            .iter()
    ...
    if !other.is_empty() && relevant.len() < 10 {
        writeln!(
            ctx,
            "  ...and {} other players with injury designations",
            other.len()
        )
        .unwrap();
    }
...
fn format_news_context(news: &Value, query_teams: &[String], query_players: &[String]) -> String {
    let mut ctx = String::new();
    ...
    let relevant_articles: Vec<&Value> = articles
        .iter()
        .filter(|article| {
...
async fn fetch_relevant_player_context(
    ...
    for (name, _id, stats_map) in relevant_stats.iter().take(max_players) {
        let stats_str: Vec<String> = stats_map
            .iter()
...
fn extract_player_mentions(query: &str) -> Vec<String> {
    ...
    for name in &found {
        let is_substring = deduped
            .iter()
            .any(|d: &&String| d.contains(name.as_str()) && d.len() > name.len());
        if !is_substring {
            deduped.push(name);
        }
    }
