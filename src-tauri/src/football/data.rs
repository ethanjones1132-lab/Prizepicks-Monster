...
pub fn data_dir() -> PathBuf {
    ...
    PathBuf::from(home).join(".openclaw/prizepicks-monster")
}

pub static DYNAMIC_FOOTBALL_CONTEXT: Lazy<Mutex<Option<FootballContext>>> =
    Lazy::new(|| Mutex::new(None));
pub static DYNAMIC_SPORT_CONTEXTS: Lazy<Mutex<HashMap<String, MultiSportContext>>> =
    ...
    if sport_upper == "NFL" || sport_upper == "FOOTBALL" {
        ...
        if let Some(players) = payload.get("players").and_then(|p| p.as_array()) {
            ...
            if !interpreted_players.is_empty() {
                ctx.top_qbs = interpreted_players
                    .iter()
                    .filter(|p| p.position.to_uppercase() == "QB")
                    .cloned()
                    .collect();
                ctx.top_rbs = interpreted_players
                    .iter()
                    .filter(|p| p.position.to_uppercase() == "RB")
                    .cloned()
                    .collect();
                ctx.top_wrs = interpreted_players
                    .iter()
                    .filter(|p| p.position.to_uppercase() == "WR")
                    .cloned()
                    .collect();
                ctx.top_tes = interpreted_players
                    .iter()
                    .filter(|p| p.position.to_uppercase() == "TE")
                    .cloned()
                    .collect();
            }
        }
        ...
        if let Some(narratives) = payload.get("narratives").and_then(|n| n.as_array()) {
            ctx.trending_narratives = narratives
                .iter()
        ...
        ctx.data_freshness = format!(
            "Injected {} Context - Updated {}",
            sport_upper,
            Utc::now().to_rfc3339()
        );
        ...
        if let Some(rankings) = payload.get("rankings").and_then(|r| r.as_array()) {
            ...
            for r in rankings {
                if let (Some(team), Some(off), Some(def)) = (
                    ...
                    r.get("offense_rank")
                        .or_else(|| r.get("offenseRank"))
                        .and_then(|o| o.as_u64()),
                    r.get("defense_rank")
                        .or_else(|| r.get("defenseRank"))
                        .and_then(|d| d.as_u64()),
                ) {
        ...
        let json = serde_json::to_string_pretty(&ctx).map_err(|e| e.to_string())?;
        fs::write(
            dir.join(format!("injected_{}.json", sport_upper.to_lowercase())),
            json,
        )
...
fn pqb(
    ...
    PlayerProfile {
        ...
        season_avg_game: HashMap::from_iter([
            ...
            ("int".into(), s_int),
            ("qbr".into(), s_qbr),
            ("rush_yds".into(), s_rush_yds),
            ("rush_td".into(), s_rush_td),
        ]),
        last_3_avg: HashMap::from_iter([("pass_yds".into(), l3_yds), ("pass_td".into(), l3_td)]),
        home_split: HashMap::from_iter([("pass_yds".into(), h_yds), ("pass_td".into(), h_td)]),
        away_split: HashMap::from_iter([("pass_yds".into(), a_yds), ("pass_td".into(), a_td)]),
        vs_top_10_def: HashMap::new(),
        vs_bottom_10_def: HashMap::new(),
        notes: notes.into(),
    }
...
fn prb(
    ...
    PlayerProfile {
        ...
        season_avg_game: HashMap::from_iter([
            ...
            ("rush_td".into(), s_td),
            ("rec".into(), s_rec),
            ("rec_yds".into(), s_rec_yds),
        ]),
        last_3_avg: HashMap::from_iter([("rush_yds".into(), l3_yds), ("rush_td".into(), l3_td)]),
        home_split: HashMap::from_iter([("rush_yds".into(), h_yds), ("rush_td".into(), h_td)]),
        away_split: HashMap::from_iter([("rush_yds".into(), a_yds), ("rush_td".into(), a_td)]),
        vs_top_10_def: HashMap::new(),
        vs_bottom_10_def: HashMap::new(),
        notes: notes.into(),
    }
...
fn build_nba_prop_categories() -> Vec<PropCategory> {
    vec![
        PropCategory {
            ...
            over_factors: s_vec(&[
                "High pace game",
                "Inefficient opponent defense",
                "Primary scoring option",
                "Hot streak",
                "Absence of other key scorer",
            ]),
            under_factors: s_vec(&[
                "Elite defender matchup",
                "Slow pace game",
                "Blowout potential (reduced minutes)",
                "Recent injury / cold streak",
            ]),
        },
        PropCategory {
            name: "Rebounds".into(),
            ...
            over_factors: s_vec(&[
                "Opponent shoots poorly (more rebound chances)",
                "Lack of dominant rebounder on own team",
                "Center playing against small-ball lineup",
            ]),
            under_factors: s_vec(&[
                "Opponent is efficient / doesn't miss",
                "Boxed out by elite rebounder (e.g., Gobert, Sabonis)",
                "Foul trouble limits minutes",
            ]),
        },
        PropCategory {
            name: "Assists".into(),
            ...
            over_factors: s_vec(&[
                ...
                "High pace game",
                "Teammates shooting well",
                "Opponent has poor transition defense",
            ]),
            under_factors: s_vec(&[
                "Playing off-ball more",
                "Matchup against elite perimeter defense",
                "Teammates are cold from the field",
            ]),
        },
        PropCategory {
            ...
            over_factors: s_vec(&[
                ...
                "Opponent allows high % from three",
                "Trailing in game script (forces more 3PA)",
            ]),
            under_factors: s_vec(&[
                "Shooting slump",
                "Matchup against elite perimeter defender",
                "Team strategy focuses on interior scoring",
            ]),
        },
    ]
}
...
fn build_mlb_context() -> MultiSportContext {
    MultiSportContext {
        sport: "MLB".to_string(),
        data_freshness:
            "Comprehensive 2025 season knowledge — 20+ player profiles, 30 team rankings."
                .to_string(),
...
fn build_mlb_prop_categories() -> Vec<PropCategory> {
    vec![
        PropCategory {
            ...
            over_factors: s_vec(&[
                "High K-rate pitcher",
                "Opponent has high K-rate (free swinging)",
                "Pitcher has long leash (high pitch count)",
            ]),
            under_factors: s_vec(&[
                "Opponent makes a lot of contact (low K-rate)",
                "Pitcher has a low pitch count limit",
                "High walk rate pitcher (drives up pitch count fast)",
            ]),
        },
        PropCategory {
            ...
            over_factors: s_vec(&[
                "Elite hitter vs. weak pitcher",
                "Favorable ballpark (e.g., Coors, GABP)",
                "Hitter has good BvP (Batter vs. Pitcher) history",
            ]),
            under_factors: s_vec(&[
                "Elite pitcher matchup",
                "Pitcher's ballpark",
                "Hitter is in a slump",
            ]),
        },
    ]
}
...
fn build_nhl_context() -> MultiSportContext {
    MultiSportContext {
        sport: "NHL".to_string(),
        data_freshness:
            "Comprehensive 2025 season knowledge — 20+ player profiles, 15 team rankings."
                .to_string(),
...
fn build_nhl_prop_categories() -> Vec<PropCategory> {
    vec![
        PropCategory {
            ...
            over_factors: s_vec(&[
                "High-volume shooter",
                "Favorable matchup vs. team that allows many shots",
                "Team is expected to be trailing (more desperate shots)",
            ]),
            under_factors: s_vec(&[
                "Player is on a defensive-minded line",
                "Matchup vs. elite defensive team that suppresses shots",
                "Blowout potential (reduced ice time)",
            ]),
        },
        PropCategory {
            name: "Points".into(),
            ...
            over_factors: s_vec(&[
                ...
                "On top Power Play unit",
                "Opponent has weak goaltending or defense",
            ]),
            under_factors: s_vec(&[
                "Matchup against top defensive line",
                "Elite goalie matchup",
                "Player is cold or has been demoted to a lower line",
            ]),
        },
    ]
}
