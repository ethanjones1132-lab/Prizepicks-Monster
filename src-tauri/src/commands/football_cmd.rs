...
#[tauri::command]
pub async fn get_game_schedule(week: Option<u32>) -> Result<Vec<data::GameInfo>, String> {
...
#[tauri::command]
pub async fn fetch_live_scoreboard() -> Result<serde_json::Value, String> {
...
#[tauri::command]
pub async fn fetch_nfl_standings() -> Result<serde_json::Value, String> {
    let client = crate::football::api_client::SportsApiClient::new(
        crate::football::api_client::SportsApiConfig::default(),
    )?;
...
#[tauri::command]
pub async fn fetch_nfl_news() -> Result<serde_json::Value, String> {
    let client = crate::football::api_client::SportsApiClient::new(
        crate::football::api_client::SportsApiConfig::default(),
    )?;
...
#[tauri::command]
pub async fn fetch_sleeper_state() -> Result<serde_json::Value, String> {
    let client = crate::football::api_client::SportsApiClient::new(
        crate::football::api_client::SportsApiConfig::default(),
    )?;
...
#[tauri::command]
pub async fn fetch_sleeper_injuries() -> Result<serde_json::Value, String> {
    let client = crate::football::api_client::SportsApiClient::new(
        crate::football::api_client::SportsApiConfig::default(),
    )?;
...
#[tauri::command]
pub async fn fetch_live_data_context(query: String) -> Result<serde_json::Value, String> {
    let client = crate::football::api_client::SportsApiClient::new(
        crate::football::api_client::SportsApiConfig::default(),
    )?;
    ...
    match client.espn_scoreboard().await {
        Ok(scoreboard) => { result["scoreboard"] = scoreboard; }
        Err(e) => { result["scoreboard_error"] = serde_json::json!(e); }
    }
    match client.espn_standings().await {
        Ok(standings) => { result["standings"] = standings; }
        Err(e) => { result["standings_error"] = serde_json::json!(e); }
    }
    match client.espn_news().await {
        Ok(news) => { result["news"] = news; }
        Err(e) => { result["news_error"] = serde_json::json!(e); }
    }
    match client.sleeper_injuries().await {
        Ok(injuries) => { result["injuries"] = injuries; }
        Err(e) => { result["injuries_error"] = serde_json::json!(e); }
    }
    match client.sleeper_news().await {
        Ok(state) => { result["nfl_state"] = state; }
        Err(e) => { result["nfl_state_error"] = serde_json::json!(e); }
    }
...
#[tauri::command]
pub async fn fetch_league_scoreboard(league: String) -> Result<serde_json::Value, String> {
    let (sport, league_code) = match league.to_lowercase().as_str() {
        ...
        _ => {
            return Err(AppError::Validation(format!(
                "Unsupported league: {}. Use NFL, NBA, MLB, or NHL.",
                league
            )).into())
        }
    };
    ...
    let resp = client.get(&url).send().await.map_err(|e| {
        AppError::Api(format!("ESPN scoreboard request failed for {}: {}", league, e))
    })?;
...
#[tauri::command]
pub async fn get_sport_league_data(league: String) -> Result<serde_json::Value, String> {
    let (sport, league_code) = match league.to_lowercase().as_str() {
        "nfl" | "football" => ("football", "nfl"),
        "nba" | "basketball" => ("basketball", "nba"),
        "mlb" | "baseball" => ("baseball", "mlb"),
    ...
    match client.get(&scoreboard_url).send().await {
        ...
        Ok(resp) => { result["scoreboard_error"] = serde_json::json!(format!("HTTP {}", resp.status())); }
        Err(e) => { result["scoreboard_error"] = serde_json::json!(e.to_string()); }
    }
    ...
    match client.get(&standings_url).send().await {
        ...
        Ok(resp) => { result["standings_error"] = serde_json::json!(format!("HTTP {}", resp.status())); }
        Err(e) => { result["standings_error"] = serde_json::json!(e.to_string()); }
    }
    ...
    match client.get(&news_url).send().await {
        ...
        Ok(resp) => { result["news_error"] = serde_json::json!(format!("HTTP {}", resp.status())); }
        Err(e) => { result["news_error"] = serde_json::json!(e.to_string()); }
    }
...
#[tauri::command]
pub async fn inject_sports_data(league: String, payload: serde_json::Value) -> Result<(), String> {
        ...
        async move {
            let league = match league_str.parse::<live_data::SportLeague>() {
                ...
                Err(e) => {
                    return (
                        league_str,
                        Err(String::from(AppError::Validation(format!("Invalid league: {}", e)))),
                    )
                }
            };
            let fetcher = match season {
