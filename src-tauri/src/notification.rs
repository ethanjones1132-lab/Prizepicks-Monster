...
impl NotificationSettings {
    pub fn normalized(self) -> Self {
        Self {
            ...
            game_starting_enabled: self.game_starting_enabled,
            game_final_enabled: self.game_final_enabled,
            prediction_graded_enabled: self.prediction_graded_enabled,
            grading_complete_enabled: self.grading_complete_enabled,
            poll_interval_secs: self.poll_interval_secs.max(15),
            game_starting_minutes_before: self.game_starting_minutes_before.clamp(1, 120),
            show_os_notifications: self.show_os_notifications,
        }
    }
}
...
impl GameStatus {
    fn from_espn(status: &str) -> Self {
        match status {
            ...
            | "STATUS_HALFTIME"
            | "STATUS_OVERTIME"
            | "STATUS_2ND_OVERTIME" => GameStatus::InProgress,
...
pub async fn get_unread_count(pool: &Pool<Sqlite>) -> Result<i64, String> {
    let row =
        sqlx::query("SELECT COUNT(*) as cnt FROM notifications WHERE read = 0 AND dismissed = 0")
            .fetch_one(pool)
            .await
...
pub async fn cleanup_old_notifications(pool: &Pool<Sqlite>, days: i64) -> Result<u64, String> {
...
impl NotificationEngine {
    ...
    pub async fn poll_cycle(&mut self, app_handle: &tauri::AppHandle) -> Result<(), String> {
        ...
        for game in &games {
            ...
            {
                self.handle_game_starting(app_handle, game, &player_predictions)
                    .await;
            }
    ...
    async fn auto_grade_for_game(
            ...
            match result.outcome.as_str() {
                "Win" | "Loss" | "Push" => {
                    ...
                    let notif = AppNotification {
                        ...
                        title: format!("{} {}: {}", emoji, result.outcome, result.player_name),
                        body: format!(
                            "{} — {} {} (Line: {}). Actual: {}",
                            result.player_name,
                            result.pick_type,
        ...
        if self.settings.grading_complete_enabled {
            let notif = AppNotification {
                ...
                body: format!(
                    "W: {} | L: {} | P: {} from game {} @ {}",
                    summary.wins, summary.losses, summary.pushes, game.away_team, game.home_team
                ),
                player_name: None,
                game_id: Some(format!("{}_{}", game.home_team, game.away_team)),
                prediction_id: None,
    ...
    async fn emit_notification(&self, app_handle: &tauri::AppHandle, notif: AppNotification) {
        // Persist to DB
...
pub fn spawn_polling_task(
    ...
    let mut current_settings = config
        .blocking_lock()
        .notification_settings
        .clone()
        .normalized();
    let mut engine = NotificationEngine::new(pool, tracker, current_settings.clone());
    ...
    tauri::async_runtime::spawn(async move {
        ...
        loop {
            tokio::time::sleep(Duration::from_secs(
                current_settings.poll_interval_secs.max(15),
            ))
            .await;

            let next_settings = config
                .lock()
                .await
                .notification_settings
                .clone()
            ...
            if next_settings != current_settings {
                engine.update_settings(next_settings.clone());
...
mod tests {
    ...
    #[test]
    fn normalized_settings_enforce_safe_polling_bounds() {
        let settings = NotificationSettings {
            poll_interval_secs: 3,
            game_starting_minutes_before: 300,
            ..NotificationSettings::default()
        }
        .normalized();

        assert_eq!(settings.poll_interval_secs, 15);
        assert_eq!(settings.game_starting_minutes_before, 120);
    }

    #[test]
    fn normalized_settings_preserve_enabled_flags() {
        let settings = NotificationSettings {
            enabled: false,
            game_starting_enabled: false,
            game_final_enabled: false,
            prediction_graded_enabled: false,
            grading_complete_enabled: false,
        ...
        assert!(!settings.game_starting_enabled);
        assert!(!settings.game_final_enabled);
        assert!(!settings.prediction_graded_enabled);
        assert!(!settings.grading_complete_enabled);
        assert!(!settings.show_os_notifications);
    }
}
