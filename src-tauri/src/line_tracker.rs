...
pub async fn snapshot_props(
    pool: &Pool<Sqlite>,
    props: &[crate::prizepicks::PrizePicksProp],
    source: &str,
    ...
    for prop in props {
        ...
        let existing: Option<i64> =
            sqlx::query_scalar("SELECT COUNT(*) FROM line_movements WHERE prop_key = ?1")
                .bind(&prop_key)
                .fetch_one(pool)
                .await
                .unwrap_or(Some(0));
...
pub async fn get_tracked_stat_categories(pool: &Pool<Sqlite>) -> Result<Vec<String>, String> {
    let rows =
        sqlx::query("SELECT DISTINCT stat_category FROM line_movements ORDER BY stat_category")
            .fetch_all(pool)
            .await
    ...
    Ok(rows
        .iter()
        .map(|r| r.get::<String, _>("stat_category"))
        .collect())
}

/// Delete old snapshots beyond a retention period (default 30 days)
pub async fn prune_old_snapshots(pool: &Pool<Sqlite>, retention_days: i64) -> Result<u64, String> {
...
pub async fn get_latest_snapshot_time(pool: &Pool<Sqlite>) -> Result<Option<String>, String> {
    let row: Option<String> = sqlx::query_scalar("SELECT MAX(snapshot_at) FROM line_movements")
        .fetch_one(pool)
        .await
        .ok();
...
mod tests {
    ...
    fn test_build_prop_key() {
        ...
        assert_eq!(
            key,
            "patrick mahomes|passing yards|nfl|2025-01-15T20:00:00Z"
        );
    }

    #[test]
    fn test_parse_prop_key() {
        let (player, stat, league, game) =
            parse_prop_key("patrick mahomes|passing yards|nfl|2025-01-15T20:00:00Z");
    ...
    fn test_parse_prop_key_no_game() {
        let (player, _stat, _league, game) =
            parse_prop_key("patrick mahomes|passing yards|nfl|unknown");
        assert_eq!(player, "patrick mahomes");
        assert_eq!(game, None);
    }
}
