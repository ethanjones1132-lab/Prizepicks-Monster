...
pub fn recommend_parlay(
    ...
    let stake = config.total_bankroll * parlay_kelly * config.kelly_fraction * 0.5; // Half Kelly for parlays
...
pub fn record_result(config: &mut BankrollConfig, stake: f64, won: bool, odds: Option<f64>) {
    let decimal = odds
        .map(american_to_decimal)
        .unwrap_or_else(|| american_to_decimal(config.default_odds));
    if won {
...
mod tests {
    ...
    fn test_staking_strategy_display() {
        assert_eq!(format!("{}", StakingStrategy::Kelly), "Kelly Criterion");
        assert_eq!(format!("{}", StakingStrategy::FlatBet), "Flat Bet");
    }
