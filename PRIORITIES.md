...
Last updated: 2026-06-29 (afternoon maintenance pass; **Per-hold-time performance breakdown shipped** — `PaperAnalytics` gained a new `hold_time_stats: Vec<PaperHoldTimeStats>` field that buckets closed lots by `closed_at - opened_at` into 4 fixed chronological buckets (Intraday ≤1h, Same day 1-24h, Multi-day 1-7d, Long >7d) plus a trailing `unknown` bucket for unparseable timestamps; `compute_hold_time_stats(&[PaperLot])` adds avg/median hold-seconds per bucket; new `PaperHoldTimeBucket` enum + `PaperHoldTimeStats` struct serialized as snake_case; 12 new unit tests cover empty input, 4-bucket classification, boundary edges (1h/24h/7d exclusive at upper end), zero-hold edge, negative-hold → unknown, unparseable timestamps, open-lot handling, push contribution, avg/median (odd/even samples), per-bucket win-rate/ROI, and bucket omission; 202 lib tests pass, tsc clean, ad-hoc verification 24/24)
Working copy: `C:\\Projects\\prizepicks-monster`
Commit: `761784e`
Quick status: **P0 done · P1 mostly done (1 partial) · P2 done · P3 done · Phase 3 partial-cache indicator done · Phase 3 combined IPC done · Phase 4 startup prefetch done · Per-category paper breakdown done · Per-side paper breakdown done · Per-window session PnL chips done · Per-hold-time paper breakdown done**
...
2. **Player-level PnL breakdown** — bucket closed lots by player name (extracted from `title` or a new column). Answers "which players am I making money on?" Requires either a parse or a schema change to store `player_name`. ~2-3h scope.
