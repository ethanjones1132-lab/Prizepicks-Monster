# Agent healthcheck for prizepicks-monster
...
Write-Host "prizepicks-monster agent healthcheck"
...
Step "PrizePicks Rust tests" {
    Push-Location (Join-Path $RepoRoot "src-tauri")
    cargo test prizepicks::
    Pop-Location
}
...
Step "Quick cache path (flat /markets)" {
    $client = Join-Path $RepoRoot "src-tauri\src\prizepicks\client.rs"
