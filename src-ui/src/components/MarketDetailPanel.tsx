...
interface Props {
  market: PrizePicksMarketSummary;
  onClose: () => void;
}

export function MarketDetailPanel({ market, onClose }: Props) {
  const [contractSide, setContractSide] = useState<'OVER' | 'UNDER'>('OVER');
  ...
  const [history, setHistory] = useState<PrizePicksPriceHistory | null>(null);
  const [historyLoading, setHistoryLoading] = useState(false);
  const [busy, setBusy] = useState(false);
  const [message, setMessage] = useState<string | null>(null);

  const marketPricePct =
    contractSide === 'OVER' ? market.yes_prob_pct : 100 - market.yes_prob_pct;
  const edgePts = fairProb - marketPricePct;

  const loadAdjustment = useCallback(async () => {
    try {
      const adj = await prizepicksApi.computeStakeAdjustment({
        ticker: market.ticker,
        category: market.category,
        contractSide: contractSide === 'OVER' ? 'YES' : 'NO',
  ...
  const recordPaperTrade = async () => {
    ...
    try {
      ...
      const decision: PrizePicksTradeDecision = {
        ticker: market.ticker,
        market_title: market.title,
        category: market.category,
        contract_side: contractSide === 'OVER' ? 'YES' : 'NO',
        ...
        <section className="modalSection tradeTicket">
          ...
          <div className="ticketRow">
            ...
            <div className="segControl">
              <button
                type="button"
                className={contractSide === 'OVER' ? 'active' : ''}
                onClick={() => setContractSide('OVER')}
              >
                OVER
              </button>
              <button
                type="button"
                className={contractSide === 'UNDER' ? 'active' : ''}
                onClick={() => setContractSide('UNDER')}
              >
                UNDER
              </button>
            </div>
